...
# ============================================
# PREDICTION: FILTERED Dataset
# ============================================

print("=" * 80)
print("PREDICTION: FILTERED Dataset (Filter Method)")
print("=" * 80)

# Load model and feature names
model = joblib.load('Models/FILTERED/model.pkl')
feature_names = joblib.load('Models/FILTERED/feature_names.pkl')

# Load test data
X_test = pd.read_csv('Data/test_FILTERED/X_test.csv')
...
# Save predictions
os.makedirs('Results/FILTERED/Predictions', exist_ok=True)
...
results.to_csv('Results/FILTERED/Predictions/predictions.csv', index=False)

print(f"\nFILTERED - Predictions completed: {len(predictions)} samples")
...
try:
    y_test = pd.read_csv('Data/test_FILTERED/y_test.csv')['num'].values
    ...
    eval_results.to_csv('Results/FILTERED/Predictions/evaluation_results.csv', index=False)
    ...
    comparison.to_csv('Results/FILTERED/Predictions/prediction_comparison.csv', index=False)
    
    print(f"\nCorrect: {sum(y_test == predictions)}/{len(predictions)}")
    
    # Display detailed predictions
    print("\n" + "="*90)
    print(f"FILTERED - DETAILED PREDICTIONS")
    print("="*90)
