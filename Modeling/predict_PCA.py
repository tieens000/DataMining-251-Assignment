...
# ============================================
# PREDICTION: PCA Dataset
# ============================================

print("=" * 80)
print("PREDICTION: PCA Dataset (Principal Component Analysis)")
print("=" * 80)

# Load model and feature names
model = joblib.load('Models/PCA/model.pkl')
feature_names = joblib.load('Models/PCA/feature_names.pkl')

# Load test data
X_test = pd.read_csv('Data/test_PCA/X_test.csv')
...
# Save predictions
os.makedirs('Results/PCA/Predictions', exist_ok=True)
...
results.to_csv('Results/PCA/Predictions/predictions.csv', index=False)

print(f"\nPCA - Predictions completed: {len(predictions)} samples")
...
try:
    y_test = pd.read_csv('Data/test_PCA/y_test.csv')['num'].values
    ...
    eval_results.to_csv('Results/PCA/Predictions/evaluation_results.csv', index=False)
    ...
    comparison.to_csv('Results/PCA/Predictions/prediction_comparison.csv', index=False)
    
    print(f"\nCorrect: {sum(y_test == predictions)}/{len(predictions)}")
    
    # Display detailed predictions
    print("\n" + "="*90)
    print(f"PCA - DETAILED PREDICTIONS")
    print("="*90)
